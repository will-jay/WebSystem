...
{
    pthread_once(&once_control, MimeType::init);//只执行一次初始化
...
void RequestData::linkTimer(shared_ptr<TimerNode> mtimer)//连接时间
{
...
void RequestData::reset()
{
    ...
    headers.clear();
    //keep_alive = false;
    if (timer.lock())//判断是否被释放
    {
        shared_ptr<TimerNode> my_timer(timer.lock());//将weak_ptr转换为强指针
        my_timer->clearReq();
        timer.reset();//将 weak_ptr 置空
    }
}
...
int RequestData::parse_URI()//分析头部
{
    string &str = inBuffer;
    // 读到完整的请求行再开始解析请求
    ...
    if (pos < 0)
    {
        ...
        if (pos < 0)
            ...
            method = METHOD_POST;//POST方法
    }
    else
        method = METHOD_GET;//get方法
    //printf("method = %d\n", method);
    // filename
...
int RequestData::parse_Headers()//分析内容
{
    string &str = inBuffer;
...
int RequestData::analysisRequest()//应答内容
{
    if (method == METHOD_POST)
    {
        //get inBuffer
        ...
        int length = stoi(headers["Content-length"]);//转换为十进制
    ...
    else if (method == METHOD_GET)
    {
        ...
        struct stat sbuf;
        if (stat(file_name.c_str(), &sbuf) < 0)//获取文件状态
        {
            header.clear();
        ...
        header += "\r\n";
        outBuffer += header;
        char *src_addr = static_cast<char*>(mmap(NULL, sbuf.st_size, PROT_READ, MAP_PRIVATE, src_fd, 0));//返回被映射区2的指针
        close(src_fd);
        outBuffer += src_addr;
        munmap(src_addr, sbuf.st_size);//解除映射关系
        return ANALYSIS_SUCCESS;
    }
    else
        return ANALYSIS_ERROR;
...
void RequestData::handleError(int fd, int err_num, string short_msg)//错误应答
{
    short_msg = " " + short_msg;
