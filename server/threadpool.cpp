...
int ThreadPool::threadpool_create(int _thread_count, int _queue_size)
{
    ...
    do
    {
        if(_thread_count <= 0 || _thread_count > MAX_THREADS || _queue_size <= 0 || _queue_size > MAX_QUEUE) //不再范围内则默认
        {
            _thread_count = 4;
            _queue_size = 1024;
        }
        ...
        thread_count = 0;//计数
        queue_size = _queue_size;//队列大小
        head = tail = count = 0;
        shutdown = started = 0;

        threads.resize(_thread_count);//设置vector大小
        queue.resize(_queue_size);//设置队列的大小
    
        /* Start worker threads */
        for(int i = 0; i < _thread_count; ++i) 
        {
            if(pthread_create(&threads[i], NULL, threadpool_thread, (void*)(0)) != 0) //创建线程
            {
                //threadpool_destroy(pool, 0);
                return -1;
            }
            ++thread_count;//增加当前线程数目
            ++started;
        }
    } while(false);
...
void myHandler(std::shared_ptr<void> req)//处理函数
{
    std::shared_ptr<RequestData> request = std::static_pointer_cast<RequestData>(req);
    if (request->canWrite())
        request->handleWrite();//回应
    else if (request->canRead())
        request->handleRead();//读取
    request->handleConn();
}

int ThreadPool::threadpool_add(std::shared_ptr<void> args, std::function<void(std::shared_ptr<void>)> fun)//往任务队列里面加任务
{
    int next, err = 0;
...
int ThreadPool::threadpool_destroy(ShutDownOption shutdown_option)//释放线程
{
...
int ThreadPool::threadpool_free()//释放锁
{
    if(started > 0)
        return -1;
